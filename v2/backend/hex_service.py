...
from .hex_model import hex_manager, BaseHex, TerrainType, SettlementHex
from .config import get_config
from .terrain_system import terrain_system
from .lore_database import LoreDatabase
from .utils.ascii_processor import process_ascii_blocks, parse_loot_section_from_ascii
import re
...
class HexService:
    ...
    def __init__(self):
        ...
        self._load_hex_data(self.current_language)

    def set_language(self, language: str):
        """Reload hex cache for a new language."""
        if language == self.current_language:
            return
        self.current_language = language
        self.hex_data_cache.clear()
        self._load_hex_data(language)
    
    def _load_hex_data(self, language: str):
        """Load all hex data from the generated JSON files."""
        hexes_dir = self.config.paths.output_path / language / "hexes"
    ...
    def _parse_hex_markdown(self, hex_file: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # If terrain is missing or 'unknown' (or old 'plains' default), use terrain_system
            if not terrain or terrain in ('unknown', 'plains'):
    ...
    def _extract_terrain(self, content: str) -> str:
        ...
        if match:
            ...
            # Normalize translated terrain names to canonical types
            #TOOO REDO THIS WITH A JSON DATABASE
            terrain_map = {
                # English (already lowercase)
                'plains': 'plains',
                'plain': 'plains',
                'mountain': 'mountain',
                'mountains': 'mountain',
            ...
            return terrain_map.get(terrain, terrain)
        return 'unknown'  # Default - changed from 'plains' to 'unknown'
    ...
    def _create_major_city_hex(self, hex_code: str, hardcoded: Dict[str, Any]) -> BaseHex:
        ...
        cities_table = database_manager.get_table('cities', 'major_cities', language)

...
class TerrainSystem:
    ...
    """
    def __init__(self, map_width: int, map_height: int, image_path: Optional[str] = None, mapping_mode: str = "letterbox", debug: bool = False, auto_size_from_image: bool = True):
        ...
        if image_path:
            ...
            if img_width and img_height:
                if auto_size_from_image:
                    # Calculate both grid dimensions from image size
                    from .utils.image_size_calculator import calculate_grid_size_from_image
                    # Use current dimensions as target (preserves user intent if they set specific size)
                    # If they want full auto-sizing, they can pass target_total_hexes or target_width
                    target_total = temp_width * temp_height
                    map_width, map_height = calculate_grid_size_from_image(
                        img_width, 
                        img_height, 
                        target_total_hexes=target_total
                    )
                    if debug:
                        print(f"[TerrainSystem] Auto-sized grid from image: {img_width}x{img_height}px -> {map_width}x{map_height} hexes")
                else:
                    # Legacy behavior: only adjust height to match aspect ratio
                ...
                # Now create ImageAnalyzer with the calculated dimensions
                self.image_analyzer = ImageAnalyzer(image_path, map_width, map_height, mapping_mode=mapping_mode, debug=debug)
    ...
    def get_terrain_for_hex(self, hex_code: str, lore_db=None) -> str:
        ...
        if lore_db:
            ...
            if hardcoded and hardcoded.get('locked', False):
                terrain = hardcoded.get('terrain', 'unknown')  # Changed from 'plains' to 'unknown' as default
                if self.debug:
    ...
    def _get_coordinate_based_terrain(self, hex_code: str, lore_db=None) -> str:
        ...
        try:
            ...
            return 'unknown'  # Changed from 'plains' to 'unknown' for invalid hex codes
        ...
        if lore_db:
            ...
            if bias:
                ...
                # Use hex_code as seed for deterministic random selection
                # Convert hex_code to integer seed (e.g., "2023" -> 2023)
                seed_value = int(hex_code) if hex_code.isdigit() else hash(hex_code) % (2**31)
                rng = random.Random(seed_value)
                return rng.choices(terrains, weights=weights, k=1)[0]
        return 'unknown'  # Changed from 'plains' to 'unknown' as final fallback

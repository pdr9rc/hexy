...
def generate_tavern_details(db_manager, language: str = 'en') -> Dict[str, Any]:
    ...
    tavern_descriptions = db_manager.get_table('descriptions', 'tavern_descriptions', language) or []
    
    # Generate tavern elements
    select_dish = random.choice(select_menu) if select_menu else {"name": "Unknown stew", "price": 4, "currency": "silver"}
    ...
    patron_trait = random.choice(patron_traits) if patron_traits else "Quiet"
    ...
    return {
        ...
        'tavern_description': random.choice(tavern_descriptions) if tavern_descriptions else None
    }
...
def generate_city_event(db_manager, language: str = 'en') -> str:
    ...
    return random.choice(city_events) if city_events else "Something unsettling happens in the streets"
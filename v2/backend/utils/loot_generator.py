...
def _pick_from_table(db_manager, category: str, table: str, language: str) -> List[Any]:
    if db_manager is None:
        return []
    data = db_manager.get_table(category, table, language)
...
class LootGenerator:
    ...
    def generate_loot(self, language: str = 'en') -> Dict[str, Any]:
        ...
        trinkets = _pick_from_table(self.db_manager, 'items_trinkets', 'trinkets', language)
        items_prices = _pick_from_table(self.db_manager, 'items_prices', 'items', language)
        weapons_prices = _pick_from_table(self.db_manager, 'weapons_prices', 'weapons', language)
        treasure_types = _pick_from_table(self.db_manager, 'loot', 'treasure_types', language)
        treasure_values = _pick_from_table(self.db_manager, 'loot', 'treasure_values', language)
        ...
        if loot_roll <= 50 and trinkets:
            ...
            loot_type = "trinket"
            effect = random.choice(_pick_from_table(self.db_manager, 'loot', 'treasure_effects', language) or ["Unknown properties"])
            value = random.choice(treasure_values) if treasure_values else None
            ttype = random.choice(treasure_types) if treasure_types else "Treasure"
        elif loot_roll <= 80 and items_prices:
            item_data = random.choice(items_prices)
            loot_item = item_data.get('name', 'Unknown item')
            loot_type = "item"
            effect = item_data.get('notes') or random.choice(_pick_from_table(self.db_manager, 'loot', 'treasure_effects', language) or ["Unknown properties"])
            value = item_data.get('value') or (random.choice(treasure_values) if treasure_values else None)
            ttype = random.choice(treasure_types) if treasure_types else "Treasure"
        elif weapons_prices:
            ...
            value = weapon_data.get('value') or (random.choice(treasure_values) if treasure_values else None)
            ttype = random.choice(treasure_types) if treasure_types else "Weapon"
        else:
        ...
        return {
            ...
            'full_description': f"{loot_type.title()}: {loot_item}. Effect: {effect}." + (f" Value: {value}." if value else "")
        }
    ...
    def generate_enhanced_loot(self, language: str = 'en') -> Dict[str, Any]:
        ...
        enhancements = _pick_from_table(self.db_manager, 'enhanced_loot', 'enhancements', language) or [

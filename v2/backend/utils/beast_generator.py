...
def generate_beast_encounter(db_manager, language: str = 'en') -> Dict[str, Any]:
    ...
    # Try to pull richer entry if bestiary has dicts with matching name
    extras = {}
    bestiary_entries = db_manager.get_table('bestiary', 'bestiary', language)
    if isinstance(bestiary_entries, list):
        match = next((b for b in bestiary_entries if isinstance(b, dict) and b.get('name') == beast_type), None)
        if match:
            extras = {k: v for k, v in match.items() if k not in ('name',)}
    return {
...
def generate_beast_description(beast_data: Dict[str, Any]) -> str:
    ...
    feature = beast_data.get('beast_feature') or random.choice(
        db_manager.get_table('bestiary', 'beast_features', language) or ['ominous presence']
    )
    behavior = beast_data.get('beast_behavior', 'hunts in the area')
    notes = beast_data.get('notes')
    stats = beast_data.get('stats')
    
    base = f"A {beast_type} with {feature} that {behavior}."
    if notes:
        base += f" {notes}"
    if stats:
        base += f" Stats: {stats}"
...
def generate_beast_markdown(beast_data: Dict[str, Any], hex_code: str) -> str:
    ...
    notes = beast_data.get('notes')
    stats = beast_data.get('stats')
    special = beast_data.get('special')

...
function loadCityContext(contextData) {
...
function getCityHexSymbol(contentType) {
...
function getCityHexTerrainClass(contentType) {
...
async function showCityDetailsInMap(app, hexCode) {
    try {
        ...
        const factions = (cityData.factions && cityData.factions.length > 0) ? cityData.factions.map((f) => `${f.name} (${f.influence}) - ${f.description}`).join("\n") : '';
        ...
        if (cityOverlayData) {
          ...
          `;
            ...
            window.handleCityHexClick = async function (hexId, district, name, description, cityHexCode) {
          ...
          `;
            ...
            `;
              ...
              <div class="ascii-section ascii-hex-related">
                ...
                <div class="ascii-content">
            `;
                            content.related_hexes.forEach((related) => {
            ...
            `;
              ...
              <div class="ascii-section ascii-hex-random">
                ...
                <div class="ascii-content">
            `;
                            content.random_table.forEach((entry) => {
                                html += `${entry}\n`;
                            });
                            html += `
                </div>
    ...
    `;
        ...
        window.filterByDistrict = function (districtName) {
        ...
        window.navigateToRelatedHex = function (hexId) {
            // Find the hex cell and trigger a click
            const hexCell = document.querySelector(`[data-hex-id="${hexId}"]`);
            if (hexCell) {
                hexCell.click();
...
async function showSettlementDetailsInMap(app, hexCode) {
    try {
        ...
        const formatMenuEntry = (entry) => {
            if (entry == null)
                return '';
            if (typeof entry === 'string')
                return entry;
            if (Array.isArray(entry))
                return entry.map(formatMenuEntry).join('\n');
...
function displaySimplifiedHexView(hexData) {
    const container = document.getElementById('details-panel');
    if (!container)
        return;
...
function displayComplexHexView(hexData) {
    const container = document.getElementById('details-panel');
    if (!container)
        return;
    console.log('🔍 Hex data for display:', hexData);
    ...
    const formatLoot = (lootData) => {
        if (typeof lootData === 'string')
            return lootData;

...
function parseSettlementFromMarkdown(md) {
    ...
    if (select_dish)
        tavern_details.select_dish = select_dish;
    if (budget_dish)
        tavern_details.budget_dish = budget_dish;
    if (innkeeper_quirk)
        tavern_details.innkeeper_quirk = innkeeper_quirk;
    if (patron_trait)
        tavern_details.patron_trait = patron_trait;
...
export async function prefetchAllHexMarkdown(onProgress) {
    const lang = getCurrentLanguage();
    const serverVersion = window.__GEN_VERSION__ || '';

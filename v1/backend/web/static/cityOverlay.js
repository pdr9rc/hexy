...
export async function showCityOverlayInMap(app, hexCode) {
    ...
    try {
        const data = await apiGet('api/city-overlays');
...
export async function showCityOverlayGridInMap(app, overlayName, hexCode) {
    ...
    try {
        const data = await apiGet(`api/city-overlay/${overlayName}`);
...
export async function showCityOverlayAsciiInMap(app, overlayName, hexCode) {
    ...
    try {
        ...
        const data = await apiGet(`api/city-overlay/${overlayName}/ascii`);
        if (data.success) {
            const mapContainer = document.querySelector('.map-container');
            const mapZoomContainer = document.getElementById('map-zoom-container');
            if (mapContainer && mapZoomContainer) {

// web/static/main.ts
import { getCityOverlay } from "./api.js";
...
let currentEditingHex = null;
let originalHexContent = null;
class DyingLandsApp {
    constructor() {
        this.mapData = {};
        this.mapWidth = 30;
    ...
    handleHexClick(hexElement) {
        ...
        const selectHexCellFn = window.selectHexCell || window.app?.selectHexCell;
        if (selectHexCellFn) {
    ...
    buildHexDetailsView(hexData, overlayName) {
    ...
    updateDistrictDetails(hexData) {
        ...
        if (this.currentCityOverlay && this.currentCityOverlay.districts) {
            const districtData = this.currentCityOverlay.districts.find((d) => d.name.toLowerCase() === district.toLowerCase());
    ...
    async regenerateHex(hexId, overlayName) {
        ...
        try {
            ...
            const result = await apiPost(`api/regenerate-hex/${targetOverlayName}/${hexId}`);
            if (result) {
                if (result.success) {
    ...
    async loadCityOverlay(overlayName) {
        try {
    ...
    async showCityOverlayInMap(hexCode) {
    ...
    getOverlayNameFromHexCode(hexCode) {
...
• Bold hexes contain special content
    ...
    // Hex editing methods
    editHexContent(hexCode) {
        console.log('🔧 Starting edit for hex:', hexCode);
        const container = document.getElementById('details-panel');
        if (!container)
            return;
        // Find the markdown content; if missing, try to fetch raw markdown and inject
        let markdownContent = container.querySelector('.markdown-content pre, .markdown-content .ascii-content');
        const ensureMarkdownBlock = async () => {
            if (markdownContent)
                return markdownContent;
            try {
                const data = await apiGet(`api/hex/${hexCode}`);
        ...
        const proceed = async () => {
            ...
            function finalize(current) {
                ...
                const saveBtn = document.getElementById('save-hex-btn');
                const cancelBtn = document.getElementById('cancel-hex-btn');
                const editBtn = container ? container.querySelector('button[onclick*="editHexContent"]') : null;
                if (saveBtn)
    ...
    async saveHexContent(hexCode) {
        console.log('💾 Saving hex content:', hexCode);
        const container = document.getElementById('details-panel');
        if (!container) {
            showError('No details panel');
            return;
        }
    ...
    cancelHexEdit() {
        ...
        const saveBtn = document.getElementById('save-hex-btn');
        const cancelBtn = document.getElementById('cancel-hex-btn');
        const editBtn = container.querySelector('button[onclick*="editHexContent"]');
        if (saveBtn)
    ...
    updateWorldMapControlsVisibility() {
    ...
    updateDistrictButtonsVisibility() {
    ...
    showCityOverlayGrid(hexCode) {
        console.log("showCityOverlayGrid called with", hexCode);
        window.showCityOverlayGrid(this, hexCode);
    }
    generateDistrictColor(districtName) {
    ...
    generateDistrictLegendRows(districts) {
...
function enableMiddleMouseDragScroll(panel) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.dyingLandsApp = appInstance;
    window.app = appInstance;
    window.showCityOverlayGrid = appInstance.showCityOverlayGrid;
    window.showCityHexDetails = appInstance.showCityHexDetails.bind(appInstance);
    window.app.showCityHexDetails = appInstance.showCityHexDetails.bind(appInstance);
    window.app.restoreMap = appInstance.restoreMap.bind(appInstance);
    window.restoreMap = appInstance.restoreMap.bind(appInstance);
    window.app.showWorldMap = appInstance.showWorldMap.bind(appInstance);
    window.showWorldMap = appInstance.showWorldMap.bind(appInstance);
    window.app.renderCityOverlay = appInstance.renderCityOverlay.bind(appInstance);
    window.renderCityOverlay = appInstance.renderCityOverlay.bind(appInstance);
    window.app.showHexDetails = appInstance.showHexDetails.bind(appInstance);
    window.showHexDetails = appInstance.showHexDetails.bind(appInstance);
    window.app.editHexContent = appInstance.editHexContent.bind(appInstance);
    window.app.saveHexContent = appInstance.saveHexContent.bind(appInstance);
    window.app.cancelHexEdit = appInstance.cancelHexEdit.bind(appInstance);
    const mapPanel = document.querySelector('.map-container');
    const mapZoomContainer = document.getElementById('map-zoom-container');
    const detailsPanel = document.querySelector('.modal-content-container');
    ...
    let zoomEventListeners = null;
    function handleMapZoom(e) {
        // Disable zooming in city view
        if (window.app && window.app.getCurrentView() === "city") {
            return;
        }
        if (e.ctrlKey)
            return; // Allow browser zoom when Ctrl is pressed
        e.preventDefault();
    ...
    function enableZoom() {
        if (!mapZoomContainer || zoomEventListeners)
            return; // Already enabled or no zoom container
        zoomEventListeners = {
            wheel: handleMapZoom,
    ...
    function disableZoom() {
        if (!mapZoomContainer || !zoomEventListeners)
            return; // Not enabled or no zoom container
    ...
    function selectHexCell(hexCode) {
        ...
        let cell = document.querySelector(`.hex-cell[data-hex="${hexCode}"]`);
        if (!cell) {
            // Try to find it within a hex container
            const container = document.querySelector(`.hex-container[data-hex="${hexCode}"]`);
            if (container) {
                cell = container.querySelector('.hex-cell');
            }
        }
        if (cell) {
    ...
    window.selectHexCell = selectHexCell;
    // Also make it available as a method on the app instance
    window.app.selectHexCell = selectHexCell;
});

...
let bleedingAnimation = null;
let bleedingCompleted = true;
let bleedingResized = false;
let bleedingClearTimeout;
function ensureOverlay() {
    let root = document.getElementById('loading-indicator');
    if (!root)
        return null;
...
function removeBleedingLines() {
    if (!bleedingCompleted)
        return;
    const svgContainer = document.getElementById('svgContainer');
    const svgGroup = svgContainer?.querySelector('g');
...
export function showLoading(_msg = 'Loading...') {
...
export function resetBleedingToTop() {
...
export const showError = (msg) => showNotification(msg, 'error');
// Custom, non-blocking confirmation shown within the loading overlay
export function showResetConfirm(onConfirm, onCancel) {
    const loading = document.getElementById('loading-indicator');
    if (!loading) {
        onConfirm();
        return;
  ...
  `;
    ...
    const yes = modal.querySelector('#confirm-reset-yes');
    const no = modal.querySelector('#confirm-reset-no');
    if (yes)
        yes.addEventListener('click', () => { cleanup(); onConfirm(); });
    if (no)
        no.addEventListener('click', () => { cleanup(); if (onCancel)
            onCancel(); });
}
export function showLoreModal(lore) {
    ...
    if (lore.major_cities) {
        html += `
          <div class="ascii-section ascii-lore-cities">
            ...
            <div class="ascii-content">${lore.major_cities.join('\n')}</div>
          </div>`;
    }
    if (lore.factions) {
        html += `
          <div class="ascii-section ascii-lore-factions">
            <span>FACTIONS</span>
            <div class="ascii-content">${lore.factions.join('\n')}</div>
          </div>`;
    }
    if (lore.notable_npcs) {
        html += `
          <div class="ascii-section ascii-lore-npcs">
            ...
            <div class="ascii-content">${lore.notable_npcs.join('\n')}</div>
          </div>`;
    }
    if (lore.regional_lore) {
        html += `
          <div class="ascii-section ascii-lore-regions">
            ...
            <div class="ascii-content">${lore.regional_lore.join('\n')}</div>
          </div>`;
    }
    html += `
        </div>
...
window.addEventListener('load', () => {
    ...
    window.startBleeding = () => startBleedingEffect();

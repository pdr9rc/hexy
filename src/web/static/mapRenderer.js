export function renderMapGrid(app) {
    ...
    headerRow.innerHTML = '<span style="margin-right: 20px;"></span>' +
        Array.from({ length: app.mapWidth }, (_, x) => `<span class="col-header" style="margin-right: 4px; font-size: 9px; display: inline-block; width: 1.5em; text-align: center;">${(x + 1).toString().padStart(2, '0')}</span>`).join('');
    ...
    for (let y = 1; y <= app.mapHeight; y++) {
        ...
        for (let x = 1; x <= app.mapWidth; x++) {
            ...
            if (hex) {
                ...
                if (hex.is_city)
                    span.classList.add('major-city');
                if (hex.content_type === 'settlement')
                    span.classList.add('settlement');
                if (hex.has_content)
